...
public class SignatureExec implements ClientExecChain {
	...
	private CloseableHttpResponse executeWithSignature(HttpRoute route, HttpRequestWrapper request,
		...
		// 添加认证信息
		String authorization = credentials.getSchema() + " " + credentials.getToken(newRequest);
		newRequest.addHeader("Authorization", authorization);
		// fanyuwen
//    newRequest.addHeader("Content-Type", "application/json");
//    newRequest.addHeader("Accept", "application/json");
//    newRequest.addHeader("Accept-Charset", "utf-8");

		// 执行
		CloseableHttpResponse response = mainExec.execute(route, HttpRequestWrapper.wrap(newRequest), context,
				execAware);
